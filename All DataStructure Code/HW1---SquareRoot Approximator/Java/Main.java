...
class Main {
  public static void main(String[] args) {
    ...
    System.out.println("please input a root: ");
    int root = scan.nextInt();
    System.out.println("The square root of " + numb + " is equal to: " +  SquareRootApproximator(numb));
    System.out.println("The "+ root + "th root of " + numb + " is equal to: " +  nthRootApproximator(numb,root));
  }

static int SquareRootApproximator(int numb){
...
static int nthRootApproximator(int numb, int root){
    if(numb == 0 || numb == 1){ //O(1)
      return numb; //O(1)
    } 
    int reset = root; //O(1)
